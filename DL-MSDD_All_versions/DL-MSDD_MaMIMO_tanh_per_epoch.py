...
SERs = []
num_samples = 100000000 # number of sequences generated
...
# tanh quantizer
def soft_quantize(y, alpha):
    return np.tanh(alpha * y.real) + 1j * np.tanh(alpha * y.imag)

# increase alpha each epoch
def get_alpha_for_epoch(epoch, total_epochs, min_alpha=1, max_alpha=20):
...
# Dataset Generation
def generate_dataset(num_samples, Nsymbols, M, SNR_list, Nu, Nrx, psp, pspRX, alpha=None):
    X = []
    ...
    for _ in range(num_samples):
        ...
        # if condition to check if we are in the training phase or testing phase
        if alpha is not None:
            # Use soft quantization for training
            yn = soft_quantize(yn, alpha)
        else:
            # Use hard quantization for testing
...
class MassiveMIMODataset(Sequence):
    def __init__(self, batch_size, steps_per_epoch, Nsymbols, M, SNR_list, Nu, Nrx, psp, pspRX, alpha):
...
# dummy generator for input dimensionality
temp_generator = MassiveMIMODataset(batch_size, 1, Nsymbols, M, EsNodBs, Nu, Nrx, psp, pspRX, alpha=1)
dummy_X, _ = temp_generator[0]
...
for epoch in range(total_epochs):
    alpha = get_alpha_for_epoch(epoch, total_epochs, min_alpha=1, max_alpha=20)
    print(f"\n=== Training epoch {epoch + 1}/{total_epochs}, alpha = {alpha:.2f} ===")
...
for EsNodB in EsNodBs:
    ...
    X_test, Y_test = generate_dataset(Online_test_samples, Nsymbols, M, np.array([EsNodB]), Nu, Nrx, psp, pspRX, alpha=None)
