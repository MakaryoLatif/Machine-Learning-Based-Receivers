...
# increase alpha each step
def get_alpha_for_step(step, total_steps, min_alpha=0.5, max_alpha=20):
...
class MassiveMIMODataset(Sequence):
    def __init__(self, batch_size, steps_per_epoch, Nsymbols, M, SNR_list, Nu, Nrx, psp, pspRX, total_steps, min_alpha=0.5, max_alpha=20):
    ...
    def __getitem__(self, idx):
        alpha = get_alpha_for_step(self.global_step, self.total_steps, self.min_alpha, self.max_alpha)
        self.global_step += 1
...
model.fit(train_generator, epochs=total_epochs, verbose=1)
